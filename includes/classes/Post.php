...
class Post {
	...
	public function submitPost($body, $user_to, $imageName) {
		...
		if($check_empty != "" || $imageName != "") {
			...
			//Audience to post to
			if ($_POST['privacyInput']=="") {
				# code...
				$post_to = $_POST['privacyInput'];
			}else{
				$post_to = $_POST['privacy'];
			}
			...
			//insert post 
			$query = mysqli_query($this->con, "INSERT INTO posts VALUES(NULL, '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0', '$imageName','$post_to')");
			if(!$query){
      ...
      if ($user_to != 'none') {
        ...
        $notification = new Notifications($this->con, $added_by);
        $notification->insertNotifications($returned_id, $user_to, "like" );
        # code...
      }
			...
			$stopWords = "a about above across after again against all almost alone along already
			 ...
			 grouping groups h had has have having he hello her here herself high high high higher
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				// if($user_logged_obj->isFriend($added_by)){
					...
					if($imagePath != "" && $imageFileType == "jpg" || $imageFileType == "jpeg" || $imageFileType == "png" || $imageFileType == "gif" || $imageFileType == "bmp") {
						$imageDiv = "<a href='$newFilePath' download><div class='postedImage'>
										<img src='$imagePath'>
									</div></a>";
					}
						...
						$imageDiv = "<div class='postedImage'>
										<embed src='$newFilePath'>
									</div>";
					}
					else if($imagePath != "" && $imageFileType == "mp4" || $imageFileType == "avi" || $imageFileType == "webm" || $imageFileType == "mkv" || $imageFileType == "mpeg"){
						$imageDiv ="<a href='$newFilePath' download><iframe width='350' height='350' src ='".$imagePath."' controls='true' frameborder='0' download>Download</iframe></a>";
					}
			...
			while($row = mysqli_fetch_array($data_query)) {
                ...
                $imagePath = $row['image'];
					...
					if($imagePath != "" && $imageFileType == "jpg" || $imageFileType == "jpeg" || $imageFileType == "png" || $imageFileType == "gif" || $imageFileType == "bmp") {
						...
						# code...
						$imageDiv = "<iframe src='$imagePath' embedded='true' frameborder='0'></iframe>";
					}
					else if($imagePath != "" && $imageFileType == "mp4" || $imageFileType == "avi" || $imageFileType == "webm" || $imageFileType == "mkv" || $imageFileType == "mpeg"){
						$imageDiv ="<video width='320' height='250' src ='".$newFilePath."' type='video/x-matroska' controls='true'></video>";
					}
					else {
						$imageDiv = "";
					}
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			$row = mysqli_fetch_array($data_query); 
				...
				if($added_by_obj->isClosed()) {
                 ...
                 //Check if user who posted, is friend 
				$user_logged_obj = new User($this->con, $userLoggedIn);
				if($user_logged_obj->isFriend($added_by)){
